...
import { MessagesSquare, Plus, Eye, MessageCircle, ThumbsUp, Pencil } from 'lucide-react';
...
export function ForumClient() {
    ...
    const [editForm, setEditForm] = useState({ forum_category_id: '1' as any, title: '', content: '', status: 'published' });
    ...
    const handleEdit = async () => {
        ...
        setSaving(true);
        try {
            await forumApi.topics.update(editingTopic.id, {
    ...
    return (
        <div className="space-y-6 animate-fadeIn font-industrial pb-20">
                                ...
                                <tr key={t.id} className="border-b border-gray-50 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-800/30 transition">
                                    ...
                                    <td className="px-5 py-4">
                                        <div className="flex gap-2">
                                            ...
                                            <button onClick={() => openEdit(t)} className="text-xs px-3 py-1.5 bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 rounded-lg hover:bg-amber-100 transition font-semibold">Editar</button>
            ...
            {/* Edit Topic Modal */}
            {editingTopic && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setEditingTopic(null)}>
                    <div className="bg-white dark:bg-[var(--bg-secondary)] rounded-3xl shadow-2xl border border-gray-100 dark:border-gray-800 w-full max-w-lg mx-4 p-6 space-y-4" onClick={e => e.stopPropagation()}>
                        <h3 className="text-lg font-bold text-gray-800 dark:text-gray-200">Editar Tema</h3>

                        <div>
                            <label className="block text-xs font-semibold text-gray-500 mb-1">Título</label>
                            <input type="text" value={editForm.title} onChange={e => setEditForm(f => ({ ...f, title: e.target.value }))} className="w-full px-4 py-2.5 border-2 border-gray-200 dark:border-gray-700 rounded-xl text-sm bg-gray-50 dark:bg-[var(--bg-primary)] text-gray-800 dark:text-gray-200" placeholder="¿Qué deseas preguntar o compartir?" />
                        </div>
                        ...
                        <div>
                            <label className="block text-xs font-semibold text-gray-500 mb-1">Contenido</label>
                            <textarea value={editForm.content} onChange={e => setEditForm(f => ({ ...f, content: e.target.value }))} rows={6} className="w-full px-4 py-2.5 border-2 border-gray-200 dark:border-gray-700 rounded-xl text-sm bg-gray-50 dark:bg-[var(--bg-primary)] text-gray-800 dark:text-gray-200" placeholder="Escribe tu contenido aquí..." />
                        </div>
                        ...
                        <div className="flex justify-end gap-3 pt-2">
                            <button onClick={() => setEditingTopic(null)} className="px-5 py-2.5 text-sm font-semibold text-gray-500 hover:text-gray-700 transition">Cancelar</button>
                            <button onClick={handleEdit} disabled={saving || !editForm.title.trim() || !editForm.content.trim()} className="px-6 py-2.5 bg-sky-500 hover:bg-sky-600 text-white rounded-xl text-sm font-semibold transition disabled:opacity-50">{saving ? 'Guardando...' : 'Guardar Cambios'}</button>
                        </div>
                    </div>
                </div>
            )}
