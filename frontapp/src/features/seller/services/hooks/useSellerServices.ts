...
export function useSellerServices() {
    ...
    const { data: services = [], isLoading: loadingServices, refetch: refetchServices } = useQuery({
        ...
        queryFn: async (): Promise<Service[]> => {
            ...
            try { return await serviceRepository.list() as unknown as Service[]; } catch (e) { console.warn('FALLBACK: Services error', e); return []; }
        },
        staleTime: 5 * 60 * 1000,
    });
    ...
    const { data: appointments = [], isLoading: loadingAppointments } = useQuery({
        ...
        queryFn: async (): Promise<AppointmentWithClient[]> => {
            ...
            try { return await bookingRepository.sellerBookings() as unknown as AppointmentWithClient[]; } catch (e) { console.warn('FALLBACK: Appointments error', e); return []; }
        },
        staleTime: 2 * 60 * 1000,
    });
