...
export function ServicesPageClient({
  ...
  // Usar los datos de TanStack Query con fallback a los iniciales provistos por el Server Component
  const activeServices = services.length > 0 ? services : initialServices;
  const activeSpecialists =
    specialists.length > 0 ? specialists : initialSpecialists;
  ...
  const saveServiceAndSyncSpecialists = (
    ...
    const futureServices: Service[] = serviceData.id
      ...
      : [...activeServices, { ...serviceData, id: -1 }]; // id temporal solo para el cálculo
  ...
  const deleteService = async (id: number) => {
    ...
    if (confirmed) {
      ...
      const remainingServices = activeServices.filter((s) => s.id !== id);
      activeSpecialists.forEach((sp) => {
  ...
  // El indicador de carga solo se despliega si está mutando datos (ya que el fetch inicial ocurre en el Servidor)
  const isMutating =
    loading && (activeServices.length > 0 || activeSpecialists.length > 0);
  ...
  return (
    <div className="space-y-8 animate-fadeIn pb-20">
      ...
      {isMutating && (
        <div className="fixed top-4 right-4 z-50 bg-sky-500 text-white px-4 py-3 rounded-2xl shadow-xl flex items-center gap-3 animate-slideInRight">
