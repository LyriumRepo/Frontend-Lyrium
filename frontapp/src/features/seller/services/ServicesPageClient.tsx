...
export function ServicesPageClient({
  ...
  const {
    ...
    handleReschedule,
  } = useSellerServices({ initialServices, initialSpecialists, initialAppointments });
  ...
  const saveServiceAndSyncSpecialists = async (
  ...
  const handlePublish = (service: Service) => {
    const nuevoEstado =
      service.estado === 'publicado' ? 'borrador' : 'publicado';
    handleSaveService({ id: service.id, estado: nuevoEstado } as Parameters<typeof handleSaveService>[0]);
    showToast(
  ...
  const specialistTotalPages = Math.max(
    1,
    Math.ceil(specialists.length / SPECIALIST_PAGE_SIZE),
  );
  ...
  return (
    <div className="space-y-8 animate-fadeIn pb-20">
      ...
      <div className="space-y-4 mt-8">
        ...
        {filteredServices.length > 0 ? (
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between px-1 pt-1">
                <p className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-widest">
                  Página {safePage} de {totalPages} · {filteredServices.length}{' '}
          ...
          <BaseEmptyState
            ...
            icon="Services"
            actionLabel={
              statusFilter === 'todos' ? 'Nuevo Servicio' : undefined
            }
            onAction={
              statusFilter === 'todos'
                ? () => {
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-[var(--border-subtle)] flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-wide">
                {specialists.length} profesional
                {specialists.length !== 1 ? 'es' : ''}
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
          {specialists.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center h-40 gap-2 text-center">
              <Icon
                name="Users"
                className="w-8 h-8 text-[var(--text-secondary)] opacity-30"
              />
        ...
        {specialistTotalPages > 1 && (
          <div className="flex-shrink-0 px-4 pt-3 pb-16 border-t border-[var(--border-subtle)] flex items-center justify-between">
            <p className="text-[9px] font-bold text-[var(--text-secondary)] uppercase tracking-widest">
              {safeSpecialistPage}/{specialistTotalPages} · {specialists.length}{' '}
              prof.
            </p>
            <div className="flex items-center gap-1">
              <button
              ...
              <button
                onClick={() =>
                  setSpecialistPage((p) =>
                    Math.min(specialistTotalPages, p + 1),
                  )
                }
      ...
      <RescheduleModal
        ...
        clients={clients}
        onClose={() =>
          setModals({ ...modals, reschedule: false, detail: true })
        }
        onConfirm={(id, fecha, sesion, specialistId) =>
          handleReschedule(id, fecha, sesion, specialistId)
        }
      />
