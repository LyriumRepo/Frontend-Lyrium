...
type ServiceWithHorarios = Service & {
  especialistaHorarios?: EspecialistaHorario[];
};
...
const JS_TO_MON: Record<number, number> = {
  1: 0,
  2: 1,
  3: 2,
  4: 3,
...
function getSpecialistWeekdays(
  service: Service,
  specialistId: number,
): Set<WeekDay> {
...
function getSpecialistBlocksForDay(
  ...
  return dayEntry.bloques.map((idx) => attDay.bloques[idx]).filter(Boolean);
}
...
export default function RescheduleModal({
  ...
  useEffect(() => {
    ...
    const handlePointerDown = (e: MouseEvent) => {
      if (
        ...
        !specialistMenuRef.current.contains(e.target as Node)
      )
        setIsSpecialistMenuOpen(false);
    };
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape') setIsSpecialistMenuOpen(false);
    };
  ...
  const custName = (appointment as Appointment & { customerName?: string }).customerName || null;
  ...
  const allSessions =
    selectedDate && selectedWeekDay && selectedSpecialistId
      ? getSpecialistBlocksForDay(
          service,
      ...
      : selectedDate
        ? (() => {
            ...
            return attDay
              ? attDay.bloques.flatMap((b) =>
                  calculateSessions(b, service.duracion),
                )
              : [];
          })()
        : [];
      ...
      <div className="p-6 space-y-5">
          ...
          >
            <div className="relative w-8 h-8 rounded-xl bg-[var(--bg-card)] flex items-center justify-center text-[10px] font-black text-sky-500 border border-[var(--border-subtle)] overflow-hidden flex-shrink-0">
              {activeSpecialist?.foto ? (
                ...
                <span>
                  {activeSpecialist ? getAvatarChars(activeSpecialist) : '??'}
                </span>
              )}
            </div>
            <div className="flex-1 min-w-0">
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <p
                            className={`truncate text-sm font-bold ${isSelected ? 'text-sky-600' : 'text-[var(--text-primary)]'}`}
                          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-7 gap-0.5">
              {DAY_HEADERS.map((h) => (
                <div
                  key={h}
                  className="text-center text-[8px] font-black text-[var(--text-secondary)] uppercase py-1"
                >
                  {h}
                </div>
              ))}
          ...
          <div className="space-y-3">
            <p className="text-[9px] font-black text-[var(--text-secondary)] uppercase tracking-widest">
              {selectedDate
                ? `Sesiones · ${formatFecha(selectedDate)}`
                : 'Sesiones disponibles'}
            </p>
            ...
            {!selectedDate ? (
              <div className="flex flex-col items-center justify-center h-[200px] rounded-2xl border border-dashed border-[var(--border-subtle)] gap-2">
                <Icon
                  name="CalendarSearch"
                  className="w-6 h-6 text-[var(--text-secondary)] opacity-30"
                />
              ...
              <div className="space-y-1.5 max-h-[200px] overflow-y-auto pr-1">
                {allSessions.map((ses) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className={`text-[11px] font-black font-mono ${isSelected ? 'text-sky-600' : 'text-[var(--text-primary)]'}`}
                      >
                        {ses.inicio} – {ses.fin}
                      </span>
                      {occupied ? (
