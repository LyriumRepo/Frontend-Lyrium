...
import { shippingApi } from '@/shared/lib/api/shippingRepository';

// NOTA: este módulo (ciudades/agencias por tienda) no tiene backend propio todavía.
// `shippingApi.storeMethods()`/`listZones()` SÍ existen y responden 200, pero devuelven
// una forma de datos distinta ({methods, zones}) a la que espera `LogisticsConfig`
// ({globalConfig, operators, cityRates}). Por eso se usa mock local en vez del flag
// global USE_MOCKS (que afecta a toda la app) hasta que exista un endpoint real con
// este modelo de datos.
const USE_REAL_LOGISTICS_API = false;
