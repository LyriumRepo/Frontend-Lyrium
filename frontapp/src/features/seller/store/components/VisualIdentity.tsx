...
type TabId = 'logos' | 'banners' | 'promos' | 'galeria';

const TABS: { id: TabId; label: string; icon: string }[] = [
    { id: 'logos',   label: 'Logos',       icon: 'Image' },
    { id: 'banners', label: 'Escaparate',   icon: 'LayoutTemplate' },
...
export default function VisualIdentity(props: VisualIdentityProps): React.ReactElement {
    ...
    return (
        <div className="glass-card p-0 overflow-hidden border-none rounded-[2rem] sm:rounded-[2.5rem] shadow-2xl bg-[var(--bg-card)] animate-fadeIn mb-4 sm:mb-6 md:mb-8">
            ...
            {/* Tab Navigation */}
            <div className="border-b border-[var(--border-subtle)] px-4 sm:px-6 md:px-8 lg:px-10">
                <nav className="flex gap-0 overflow-x-auto no-scrollbar" role="tablist">
                    {TABS.map((tab) => (
                        <button
                            key={tab.id}
                            role="tab"
                            ...
                            className={`flex items-center gap-2 px-4 sm:px-5 py-3 sm:py-4 text-[10px] sm:text-[11px] font-semibold uppercase tracking-widest whitespace-nowrap border-b-2 transition-all ${
                                activeTab === tab.id
                                    ? 'border-[var(--brand-green)] text-[var(--brand-green)] dark:text-[var(--icons-green)] dark:border-[var(--icons-green)]'
            ...
            <div className="p-4 sm:p-6 md:p-8 lg:p-10">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 sm:gap-12 max-w-3xl mx-auto">
                        ...
                        <div className="flex flex-col items-center">
                            <div className="flex items-center gap-2 mb-6">
                                ...
                                <span className="text-[11px] font-semibold text-[var(--text-primary)] uppercase tracking-widest">Logo de Marca</span>
                            </div>
                            <div
                                role="button"
                                tabIndex={0}
                                className="relative w-36 h-36 sm:w-44 sm:h-44 bg-[var(--bg-secondary)] rounded-full ring-4 ring-[var(--bg-secondary)] ring-offset-4 ring-offset-[var(--bg-card)] border-2 border-dashed border-[var(--border-subtle)] group cursor-pointer overflow-hidden flex items-center justify-center transition-all duration-500 hover:scale-[1.02] hover:border-sky-400 dark:hover:border-[var(--icons-green)]"
                            ...
                            >
                                {localLogo ? (
                                    <Image src={localLogo} fill sizes="(max-width: 640px) 144px, 176px" alt="Logo" className="object-contain group-hover:scale-110 transition-transform duration-700" />
                                ) : (
                            ...
                            <div className="mt-6 space-y-2 text-center">
                                <div className="flex items-center justify-center gap-2">
                                    ...
                                    <p className="text-[10px] font-semibold text-[var(--text-secondary)] uppercase">Ratio 1:1</p>
                                </div>
                                <p className="text-[9px] font-semibold text-red-400 uppercase">PNG &bull; JPG &bull; WEBP (Máx 2MB)</p>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col items-center">
                            <div className="flex items-center gap-2 mb-6">
                                ...
                                <span className="text-[11px] font-semibold text-[var(--text-primary)] uppercase tracking-widest">Logo para Tarjetas</span>
                            </div>
                            <div
                                role="button"
                                tabIndex={0}
                            ...
                            >
                                {localLogoMarketplace ? (
                                    <Image src={localLogoMarketplace} fill sizes="(max-width: 640px) 144px, 176px" alt="Logo Marketplace" className="object-contain group-hover:scale-110 transition-transform duration-700" />
                                ) : (
                                    <Icon name="Image" className="w-10 h-10 sm:w-12 sm:h-12 text-[var(--text-secondary)]" />
                                )}
                                <div className="absolute inset-0 bg-amber-600/60 dark:bg-[var(--icons-green)] backdrop-blur-[0px] opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col items-center justify-center text-white">
                                    <Icon name="Camera" className="w-7 h-7 sm:w-8 sm:h-8 mb-2 animate-bounce" />
                                    <span className="text-[9px] font-semibold uppercase tracking-widest">{uploading === 'logo-marketplace' ? 'Subiendo...' : 'Actualizar'}</span>
                                </div>
                                <input
                                    type="file"
                ...
                {/* ══ TAB: Escaparate de Banners ═══════════════════════════ */}
                {activeTab === 'banners' && (
                    <div className="space-y-6">
                        <div className="flex flex-wrap items-center justify-between gap-y-2">
                            <p className="text-[10px] font-semibold text-[var(--text-secondary)] leading-relaxed">
                                Banners principales que se muestran en la parte superior de tu tienda pública.
                            </p>
                            <p className="text-[9px] font-semibold text-red-400 uppercase tracking-widest italic flex items-center gap-1">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                            ...
                            >
                                ...
                                <div className="absolute top-3 left-3 sm:top-4 sm:left-4">
                                    <span className="px-2 py-1 bg-black/40 backdrop-blur-md rounded-lg text-[8px] font-semibold text-white uppercase tracking-tighter border border-white/20">Banner Principal</span>
                                </div>
                                <input
                                    type="file"
                                    id="input-banner1"
                                ...
                                <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 flex items-end p-4 sm:p-6">
                                    <span className="text-white text-[10px] font-semibold uppercase tracking-widest flex items-center gap-2">
                                    ...
                                    >
                                        {locked ? (
                                            ...
                                            <>
                                                ...
                                                <div className="absolute top-3 left-3 sm:top-4 sm:left-4">
                                                    <span className="px-2 py-1 bg-black/40 backdrop-blur-md rounded-lg text-[8px] font-semibold text-white uppercase tracking-tighter border border-white/20">{label}</span>
                                                </div>
                                                <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 flex items-end p-4 sm:p-6">
                                                    <span className="text-white text-[10px] font-semibold uppercase tracking-widest flex items-center gap-2">
                                                        <Icon name="Upload" className="w-4 h-4" />
                                            ...
                                            <div className="text-center px-4">
                                                ...
                                                <p className="text-[10px] font-semibold text-sky-600 dark:text-[var(--icons-green)] uppercase tracking-widest">Expandir Campaña</p>
                                                <p className="text-[8px] font-semibold text-sky-300 dark:text-[var(--icons-green)] uppercase mt-1">Añadir {label}</p>
                                            </div>
                                        )}
                                        <input
                                            type="file"
                ...
                {activeTab === 'galeria' && (
                    <div className="space-y-6">
                        <div className="flex flex-wrap items-center justify-between gap-y-3">
                            <p className="text-[10px] font-semibold text-[var(--text-secondary)] leading-relaxed">
                                Imágenes de instalaciones, productos o experiencias de tu tienda.
                            </p>
                            <button
                                className="flex items-center gap-2 px-4 sm:px-5 py-2 sm:py-2.5 bg-sky-500/10 dark:bg-emerald-500/10 text-sky-500 dark:text-[var(--icons-green)] rounded-2xl font-semibold text-[10px] uppercase tracking-widest hover:bg-sky-500 dark:hover:bg-[var(--brand-green)] hover:text-white transition-all"
                        ...
                        <div className="flex flex-wrap gap-x-6 gap-y-6 sm:gap-x-10 sm:gap-y-8 md:gap-x-12 md:gap-y-10 pt-4 pl-2 sm:pl-4">
                            ...
                            >
                                ...
                                <span className="text-[8px] font-semibold text-[var(--text-secondary)] uppercase mt-1">Próxima Foto</span>
                                <input
                                    type="file"
                                    id="input-gallery"
                                    className="hidden"
