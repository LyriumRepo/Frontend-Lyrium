...
export default function BranchCard({ branch, onEdit, onDelete }: BranchCardProps) {
    return (
        <div className="p-6 bg-[var(--bg-card)] border border-[var(--border-subtle)] rounded-3xl relative group hover:shadow-xl hover:shadow-black/5 transition-all">
            {branch.isPrincipal && (
                <span className="absolute top-4 right-4 bg-sky-500 dark:bg-[var(--icons-green)] text-white text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-lg">
                    Principal
                </span>
            )}
            <h4 className="font-black text-[var(--text-primary)] flex items-center gap-2">
                <Icon name="Store" className="text-sky-500 dark:text-[var(--icons-green)] w-4 h-4" /> {branch.name}
            </h4>

            <div className="space-y-2 mt-4 mb-6">
                <div className="flex items-center gap-2 text-xs font-bold text-[var(--text-secondary)]">
                    <Icon name="MapPin" className="text-sky-400 dark:text-[var(--icons-green)] w-4 h-4" />
                ...
                <div className="flex items-center gap-2 text-xs font-bold text-[var(--text-secondary)]">
                    <Icon name="Clock" className="text-sky-400 dark:text-[var(--icons-green)] w-4 h-4" />
