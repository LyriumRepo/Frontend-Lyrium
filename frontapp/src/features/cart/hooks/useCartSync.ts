...
import {
  cartApi,
  type CartResource,
  type CartItem as ApiCartItemResponse,
} from "@/shared/lib/api/cartRepository";
...
export function useCartSync(): UseCartSyncReturn {
  ...
  const addToCart = useCallback(
    async (product: Producto, cantidad = 1) => {
  ...
  const removeFromCart = useCallback(
    async (productId: number | string) => {
      const localItem = localItems.find(
        (i) => String(i.producto_id) === String(productId),
      );
      if (localItem) {
  ...
  const itemCount = localItems.reduce(
    (sum, item) => sum + Number(item.cantidad ?? 0),
    0,
  );
  const subtotal = localItems.reduce(
    (sum, item) =>
      sum + Number(item.precio_unitario) * Number(item.cantidad ?? 0),
    0,
  );
