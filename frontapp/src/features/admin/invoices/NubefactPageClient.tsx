...
export function NubefactPageClient(_props: NubefactPageClientProps) {
    ...
    return (
        <main className="p-8 space-y-8 animate-fadeIn">
            ...
            {isLoading ? (
                <div className="space-y-8"><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">{[1, 2, 3, 4].map((i) => <Skeleton key={`nubefact-skel-${i}`} className="h-28 rounded-[2rem]" />)}</div></div>
            ) : (
                <>
                    ...
                    <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden flex flex-col">
                        <div className="p-8 border-b border-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-6 bg-gray-50/30">
                            <div><h3 className="text-xl font-black text-gray-900 tracking-tight">Comprobantes Recientes</h3><p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">Sincronizado con NubeFact</p></div>
