...
import {
  UnifiedTicketListItem,
  adaptAdminTicketList,
} from "@/modules/helpdesk";
...
function sortTicketsByRecentActivity(tickets: Ticket[]): Ticket[] {
  return [...tickets].sort((left, right) => {
    const leftTime = left.fecha_actualizacion
      ? new Date(left.fecha_actualizacion).getTime()
      : 0;
    const rightTime = right.fecha_actualizacion
      ? new Date(right.fecha_actualizacion).getTime()
      : 0;
...
export const useMesaAyuda = () => {
  ...
  const updateCachedTicket = useCallback(
    (ticketId: number, patch: Partial<Ticket>) => {
      queryClient.setQueryData(
        ["admin", "helpdesk"],
        (old: MesaAyudaData | undefined) => {
          if (!old) {
            return old;
          }
          ...
          return {
            ...old,
            tickets: sortTicketsByRecentActivity(
              old.tickets.map((ticket) =>
  ...
  const refreshTicketData = async (ticketId?: number | null) => {
    ...
    if (ticketId) {
      await queryClient.invalidateQueries({
        queryKey: ["admin", "helpdesk", "ticket", ticketId],
      });
  ...
  const sendReplyMutation = useMutation({
    mutationFn: async ({
      ...
      return ticketApi.admin.sendMessage(selectedTicketId, {
  ...
  const selectedTicket = useMemo(() => {
    const summaryTicket =
      helpdeskData?.tickets.find((ticket) => ticket.id === selectedTicketId) ||
      null;
  ...
  const filteredTickets = useMemo(() => {
    ...
    return tickets.filter((ticket) => {
      ...
      const matchPriority =
        !filters.priority || ticket.prioridad === filters.priority;
  ...
  const normalizedTickets = filteredTickets.map((ticket) => ({
    ...ticket,
    estado: ticket.estado ?? "abierto",
    mensajes_sin_leer: ticket.mensajes_sin_leer ?? 0,
    total_mensajes: ticket.total_mensajes ?? 0,
    prioridad: ticket.prioridad ?? "media",
    fecha_actualizacion: ticket.fecha_actualizacion ?? "",
  }));

  const unifiedTickets: UnifiedTicketListItem[] =
    adaptAdminTicketList(normalizedTickets);
  ...
  return {
    ...
    actions: {
      ...
      assignAdmin: (adminId: number) => {
        if (selectedTicketId) {
          void assignAdminMutation.mutateAsync({
            ticketId: selectedTicketId,
            adminId,
          });
        }
      ...
      escalateTicket: (reason?: string) => {
        if (selectedTicketId) {
          void escalateTicketMutation.mutateAsync({
            ticketId: selectedTicketId,
            reason,
          });
        }
      ...
      loadMoreMessages: async () => {
        ...
        const detail = queryClient.getQueryData<Ticket>([
          "admin",
          "helpdesk",
          "ticket",
          selectedTicketId,
        ...
        try {
          const { messages: older, hasMore } =
            await ticketApi.admin.getMessages(selectedTicketId, oldestId);
          queryClient.setQueryData(
            ...
            (old: Ticket | null | undefined) => {
              ...
              return {
                ...
                oldest_message_id:
                  older.length > 0 ? older[0].id : old.oldest_message_id,
