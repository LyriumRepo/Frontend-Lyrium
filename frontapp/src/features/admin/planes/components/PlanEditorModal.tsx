...
export default function PlanEditorModal({ open, title, activeTab, editingPlan: ep, editFeatures, editDetailedBenefits, onClose, onSave, onTabChange, onUpdatePlan, onAddFeature, onUpdateFeature, onRemoveFeature, onAddDetailedBenefit, onUpdateDetailedBenefit, onRemoveDetailedBenefit, onImageUpload }: Props) {
  ...
  return (
    <Modal open={open} onClose={onClose} className="plan-editor-modal">
      <h2 className="text-2xl font-extrabold text-gray-800 dark:text-[var(--text-primary)] mb-5">{title}</h2>
      ...
      <div className="editor-content max-h-[60vh] overflow-y-auto pr-1">
          ...
          <div className="block animate-fade-in">
            <div className="p-5 bg-gray-50 dark:bg-[var(--bg-muted)] rounded-xl">
              ...
              <div className="mb-5">
                ...
                <p className="text-xs text-gray-400 dark:text-[var(--text-placeholder)] mt-2">Desactiva para usar texto personalizado (ej: "Prueba gratuita")</p>
              </div>
              {showNumeric ? (
                <>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="mb-4">
                      <label htmlFor="plan-price-annual" className="block text-xs font-semibold text-gray-500 dark:text-[var(--text-secondary)] uppercase tracking-wide mb-1.5">Precio Anual</label>
                      <input id="plan-price-annual" type="number" value={p.priceAnnual ?? 0} placeholder="0.00" step={0.01} min={0} className="w-full px-3.5 py-2.5 border-2 border-gray-200 dark:border-[var(--border-subtle)] rounded-lg text-sm"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="mb-4">
                      <label htmlFor="plan-period-annual" className="block text-xs font-semibold text-gray-500 dark:text-[var(--text-secondary)] uppercase tracking-wide mb-1.5">Etiqueta Anual</label>
                      <input id="plan-period-annual" type="text" value={p.periodAnnual ?? '/año'} placeholder="/año" maxLength={20} className="w-full px-3.5 py-2.5 border-2 border-gray-200 dark:border-[var(--border-subtle)] rounded-lg text-sm"
                ...
                <>
                  <div className="mb-4">
                    <label htmlFor="plan-price-text" className="block text-xs font-semibold text-gray-500 dark:text-[var(--text-secondary)] uppercase tracking-wide mb-1.5">Texto Principal de Precio</label>
                    <input type="text" value={p.priceText ?? ''} placeholder="Ej: Prueba gratuita" className="w-full px-3.5 py-2.5 border-2 border-gray-200 dark:border-[var(--border-subtle)] rounded-lg text-sm"
                      onChange={e => onUpdatePlan({ priceText: e.target.value })} />
                  </div>
                  <div className="mb-4">
                    <label htmlFor="plan-price-subtext" className="block text-xs font-semibold text-gray-500 dark:text-[var(--text-secondary)] uppercase tracking-wide mb-1.5">Subtexto (opcional)</label>
                    <input id="plan-price-subtext" type="text" value={p.priceSubtext ?? ''} placeholder="Ej: 6 meses" className="w-full px-3.5 py-2.5 border-2 border-gray-200 dark:border-[var(--border-subtle)] rounded-lg text-sm"
        ...
        {activeTab === 'design' && (
          <div className="block animate-fade-in">
            ...
            <div className="p-5 bg-gray-50 dark:bg-[var(--bg-muted)] rounded-xl">
              <h3 className="text-base font-bold text-gray-800 dark:text-[var(--text-primary)] mb-4">Imagen de Fondo</h3>
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-2.5">
                  ...
                  <div className="flex items-center gap-2">
                    <label htmlFor="plan-bg-position" className="text-xs font-semibold text-gray-500 dark:text-[var(--text-secondary)] whitespace-nowrap">Posición:</label>
                    <select id="plan-bg-position" className="flex-1 px-2.5 py-1.5 border border-gray-200 dark:border-[var(--border-subtle)] rounded-md text-xs font-semibold text-gray-700 dark:text-[var(--text-primary)] cursor-pointer" value={p.bgImagePosition ?? 'center'} onChange={e => onUpdatePlan({ bgImagePosition: e.target.value })}>
                ...
                {p.bgImage && (
                  <div className="mt-2.5 rounded-xl overflow-hidden h-40 bg-gray-100 dark:bg-[var(--bg-muted)] relative">
                    <Image 
                      src={p.bgImage} 
                      alt="Preview" 
                      fill
                ...
                <p className="text-xs text-gray-400 dark:text-[var(--text-placeholder)] mt-2">Acepta PNG, JPG, WebP, SVG, GIF, AVIF, HEIC y más.</p>
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'features' && (
          <div className="block animate-fade-in">
            <div className="p-5 bg-gray-50 dark:bg-[var(--bg-muted)] rounded-xl">
              <h3 className="text-base font-bold text-gray-800 dark:text-[var(--text-primary)] mb-2">Beneficios Compactos (Tarjeta)</h3>
              <p className="text-xs text-gray-400 dark:text-[var(--text-placeholder)] mb-4">Aparecen en la tarjeta del plan. Elige cuántos mostrar antes del "Ver más".</p>
              <div className="flex flex-wrap items-center gap-3 mb-4 p-3 bg-white dark:bg-[var(--bg-card)] border border-gray-200 dark:border-[var(--border-subtle)] rounded-xl">
                <span className="text-xs font-semibold text-gray-600 dark:text-[var(--text-secondary)]">Mostrar</span>
                <input type="number" value={visibleCount} min={1} max={20} className="w-14 px-2 py-1.5 border border-gray-200 dark:border-[var(--border-subtle)] rounded-md text-sm font-semibold text-center"
                  onChange={e => onUpdatePlan({ compactVisibleCount: parseInt(e.target.value) || 5 })} />
                <span className="text-xs font-semibold text-gray-600 dark:text-[var(--text-secondary)]">antes del "Ver más"</span>
                <div className="ml-auto flex gap-1.5">
                  <button className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${featFilter === 'all' ? 'bg-gray-800 text-white' : 'bg-gray-100 dark:bg-[var(--bg-muted)] text-gray-500 dark:text-[var(--text-secondary)] hover:bg-gray-200'}`}
                    onClick={() => setFeatFilter('all')}>Todos</button>
                  <button className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all flex items-center gap-1 ${featFilter === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 dark:bg-[var(--bg-muted)] text-gray-500 dark:text-[var(--text-secondary)] hover:bg-gray-200'}`}
...
{/* DETALLADOS */}
        {activeTab === 'detailed' && (
          <div className="block animate-fade-in">
            <div className="p-5 bg-gray-50 dark:bg-[var(--bg-muted)] rounded-xl">
              <h3 className="text-base font-bold text-gray-800 dark:text-[var(--text-primary)] mb-2">Beneficios Detallados (Modal)</h3>
        ...
        {activeTab === 'config' && (
          <div className="block animate-fade-in">
            <div className="p-5 bg-gray-50 dark:bg-[var(--bg-muted)] rounded-xl mb-5">
              ...
              <p className="text-xs text-gray-400 dark:text-[var(--text-placeholder)] mt-2">Desactiva para planes completamente gratuitos o de prueba</p>
            </div>
            <div className="p-5 bg-gray-50 dark:bg-[var(--bg-muted)] rounded-xl mb-5">
              ...
              <div className="mb-3">
                <label htmlFor="plan-claimed-button-text" className="block text-xs font-semibold text-gray-500 dark:text-[var(--text-secondary)] uppercase tracking-wide mb-1.5">Texto del Botón Bloqueado</label>
                <input id="plan-claimed-button-text" type="text" value={p.claimedButtonText ?? ''} placeholder="Este plan ya ha sido reclamo con anterioridade" className="w-full px-3.5 py-2.5 border-2 border-gray-200 dark:border-[var(--border-subtle)] rounded-lg text-sm"
                  onChange={e => onUpdatePlan({ claimedButtonText: e.target.value })} />
              </div>
              <div className="mb-3">
                <label htmlFor="plan-claimed-warning-text" className="block text-xs font-semibold text-gray-500 dark:text-[var(--text-secondary)] uppercase tracking-wide mb-1.5">Mensaje de advertencia</label>
                <input id="plan-claimed-warning-text" type="text" value={p.claimedWarningText ?? ''} placeholder="⚠ Este plan solo puede ser reclamo una única vez." className="w-full px-3.5 py-2.5 border-2 border-gray-200 dark:border-[var(--border-subtle)] rounded-lg text-sm"
                  onChange={e => onUpdatePlan({ claimedWarningText: e.target.value })} />
              </div>
              <div className="mb-3">
                <label htmlFor="plan-claim-months" className="block text-xs font-semibold text-gray-500 dark:text-[var(--text-secondary)] uppercase tracking-wide mb-1.5">Duración del plan gratuito (meses)</label>
                <input id="plan-claim-months" type="number" value={p.claimMonths ?? 1} min={1} max={120} className="w-full px-3.5 py-2.5 border-2 border-gray-200 dark:border-[var(--border-subtle)] rounded-lg text-sm"
                  onChange={e => onUpdatePlan({ claimMonths: parseInt(e.target.value) || 1 })} />
              </div>
</div>
            <div className="p-5 bg-gray-50 dark:bg-[var(--bg-muted)] rounded-xl">
              <h3 className="text-base font-bold text-gray-800 dark:text-[var(--text-primary)] mb-4">Textos Personalizables</h3>
              <div className="mb-3">
                <label htmlFor="plan-subscribe-button-text" className="block text-xs font-semibold text-gray-500 dark:text-[var(--text-secondary)] uppercase tracking-wide mb-1.5">Texto del Botón de Suscripción</label>
                <input id="plan-subscribe-button-text" type="text" value={p.subscribeButtonText ?? 'Suscribirse'} placeholder="Suscribirse" className="w-full px-3.5 py-2.5 border-2 border-gray-200 dark:border-[var(--border-subtle)] rounded-lg text-sm"
                  onChange={e => onUpdatePlan({ subscribeButtonText: e.target.value })} />
              </div>
              <div className="mb-3">
                <label htmlFor="plan-trial-success-title" className="block text-xs font-semibold text-gray-500 dark:text-[var(--text-secondary)] uppercase tracking-wide mb-1.5">Título del Modal de Éxito</label>
                <input id="plan-trial-success-title" type="text" value={p.trialSuccessTitle ?? ''} placeholder="¡Solicitud enviada!" className="w-full px-3.5 py-2.5 border-2 border-gray-200 dark:border-[var(--border-subtle)] rounded-lg text-sm"
                  onChange={e => onUpdatePlan({ trialSuccessTitle: e.target.value })} />
              </div>
              <div className="mb-3">
                <label htmlFor="plan-trial-success-message" className="block text-xs font-semibold text-gray-500 dark:text-[var(--text-secondary)] uppercase tracking-wide mb-1.5">Mensaje Principal del Modal</label>
                <textarea id="plan-trial-success-message" value={p.trialSuccessMessage ?? ''} rows={2} placeholder="Prueba gratuita de 6 meses..." className="w-full px-3.5 py-2.5 border-2 border-gray-200 dark:border-[var(--border-subtle)] rounded-lg text-sm resize-y"
                  onChange={e => onUpdatePlan({ trialSuccessMessage: e.target.value })} />
              </div>
              <div className="mb-3">
                <label htmlFor="plan-trial-wait-message" className="block text-xs font-semibold text-gray-500 dark:text-[var(--text-secondary)] uppercase tracking-wide mb-1.5">Mensaje de Espera</label>
                <textarea id="plan-trial-wait-message" value={p.trialWaitMessage ?? ''} rows={2} placeholder="Espere la aceptación del administrador..." className="w-full px-3.5 py-2.5 border-2 border-gray-200 dark:border-[var(--border-subtle)] rounded-lg text-sm resize-y"
        ...
        <div className="flex gap-3 justify-end pt-5">
          <button className="px-6 py-3 border-2 border-gray-200 dark:border-[var(--border-subtle)] bg-transparent text-gray-600 dark:text-[var(--text-secondary)] rounded-xl text-sm font-bold cursor-pointer transition-all hover:border-gray-400 hover:text-gray-800"
