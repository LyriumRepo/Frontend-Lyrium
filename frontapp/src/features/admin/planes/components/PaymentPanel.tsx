      ...
      {open && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pt-4 px-1">
          {txs.map(tx => {
            ...
            return (
              <div key={tx.id} className="bg-[var(--bg-card)] rounded-xl shadow-md overflow-hidden border border-[var(--border-subtle)] transition-transform hover:-translate-y-0.5">
                ...
                <div className="p-4">
                  ...
                  <div className="border-t-2 border-dashed border-[var(--border-subtle)] my-3" />
                  <div className="flex justify-between text-xs mb-2">
                    <span className="text-[var(--text-placeholder)] font-medium">Plan</span><span className="text-[var(--text-primary)] font-semibold text-right max-w-[58%] break-all" style={{ color:tx.planColor, fontWeight:800 }}>{tx.planNombre ?? tx.planId}</span>
                  </div>
                  <div className="flex justify-between text-xs mb-2">
                    <span className="text-[var(--text-placeholder)] font-medium">Vendedor</span><span className="text-[var(--text-primary)] font-semibold text-right max-w-[58%] break-all">{vendedor.username ?? '—'}</span>
                  </div>
                  <div className="flex justify-between text-xs mb-2">
                    <span className="text-[var(--text-placeholder)] font-medium">Duración</span><span className="text-[var(--text-primary)] font-semibold">{tx.meses} mes(es)</span>
                  </div>
                  <div className="flex justify-between text-xs mb-2">
                    <span className="text-[var(--text-placeholder)] font-medium">Fecha</span><span className="text-[var(--text-primary)] font-semibold">{fechaStr}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-[var(--text-placeholder)] font-medium">Hora</span><span className="text-[var(--text-primary)] font-semibold">{horaStr}</span>
                  </div>
                  {tx.procesadoEn && (() => {
                    const p = new Date(tx.procesadoEn!);
                    return <div className="flex justify-between text-xs mt-2"><span className="text-[var(--text-placeholder)] font-medium">Procesado</span><span className="text-[var(--text-primary)] font-semibold">{p.toLocaleDateString('es-PE', locOpts)} {p.toLocaleTimeString('es-PE', timeOpts)}</span></div>;
                  })()}
                  {tx.transactionId && <div className="flex justify-between text-xs mt-2"><span className="text-[var(--text-placeholder)] font-medium">ID Izipay</span><span className="text-[var(--text-secondary)] font-mono text-[10px] break-all">{tx.transactionId}</span></div>}
                </div>
                <div className="bg-[var(--bg-muted)] border-t border-[var(--border-subtle)] px-4 py-2 text-[10px] text-[var(--text-placeholder)] text-center tracking-wide uppercase">Procesado por <strong className="text-[var(--text-secondary)]">Izipay</strong></div>
              </div>
            );
          })}
        </div>
    ...
    <>
      ...
      <div className="mt-4">
        <AdminTable
          ...
          mobileCardRender={(v) => <BoletaGrid vendedor={v} filter={filter} />}
        />
      </div>
    </>
  );
...
function paymentColumns(filter: string): Column<VendedorPago>[] {
  return [
    {
      key: 'vendedor',
      header: 'Vendedor',
      render: (v) => <BoletaGrid vendedor={v} filter={filter} />,
    },
  ];
}
