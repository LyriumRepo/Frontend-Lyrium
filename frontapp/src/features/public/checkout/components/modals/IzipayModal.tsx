...
export default function IzipayModal({ isOpen, onClose, error }: Props) {
    ...
    return (
        <>
            <div
                className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[20000] animate-fade-in"
                onClick={onClose}
            />

            <div className="fixed inset-0 z-[30000] flex items-center justify-center p-4">
                <div
                    className="bg-white dark:bg-[var(--bg-card)] w-full max-w-sm rounded-2xl shadow-2xl border border-gray-100 dark:border-[var(--border-subtle)] overflow-hidden animate-slide-up"
