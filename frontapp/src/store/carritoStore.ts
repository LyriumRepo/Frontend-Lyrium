...
interface CarritoUI {
  ...
  popupOpen: boolean; // ← NUEVO
}

export interface LastAddedService {
  name: string;
  price: number;
  image?: string | null;
...
interface CarritoState {
  ...
  openPopup: () => void; // ← NUEVO
  closePopup: () => void; // ← NUEVO
...
function applySort(list: ApiProduct[], sortBy: SortOption): ApiProduct[] {
  ...
  switch (sortBy) {
    ...
    case 'ratingDesc':
      return arr.sort(
        (a, b) =>
          Number(b.rating_promedio ?? 0) - Number(a.rating_promedio ?? 0),
      );
    default:
      return arr;
  }
...
export const useCarritoStore = create<CarritoState>((set, get) => ({
  ...
  setProducts: (products) => {
    ...
    set({
      products,
      priceBoundMin: boundMin,
      priceBoundMax: boundMax,
      priceMin: boundMin,
      priceMax: boundMax,
    });
    get().applyFilters();
  ...
  setPriceBounds: (priceBoundMin, priceBoundMax) =>
    set({ priceBoundMin, priceBoundMax }),
  resetPriceFilter: () => {
  ...
  openDetailModal: (id) =>
    set((s) => ({
      ui: { ...s.ui, detailModalOpen: true, detailProductId: id },
    })),
  closeDetailModal: () =>
    set((s) => ({
      ui: { ...s.ui, detailModalOpen: false, detailProductId: null },
    })),

  // ── Popup ──────────────────────────────────────────────────────────────────
  openPopup: () => set((s) => ({ ui: { ...s.ui, popupOpen: true } })),
  closePopup: () => set((s) => ({ ui: { ...s.ui, popupOpen: false } })),
  ...
  addToCart: (product, cantidad = 1) => {
    ...
    const existingItem = cartItems.find(
      (item) => String(item.producto_id) === productId,
    );
  ...
  removeFromCart: (productId) => {
    ...
    set({
      cartItems: cartItems.filter(
        (item) => String(item.producto_id) !== String(productId),
      ),
    });
  },
