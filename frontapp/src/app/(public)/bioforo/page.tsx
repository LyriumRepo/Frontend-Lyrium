...
export default function BioForoPage() {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-3 md:px-4 py-4 md:py-8 space-y-8 md:space-y-12">
      ...
      {shareTopic && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setShareTopic(null)} onKeyDown={(e) => { if (e.key === 'Escape') setShareTopic(null); }} role="dialog" aria-modal="true" tabIndex={-1}>
          <div className="bg-white dark:bg-[var(--bg-secondary)] rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-800 w-full max-w-sm mx-4 p-6" onClick={e => e.stopPropagation()} onKeyDown={e => e.stopPropagation()}>
