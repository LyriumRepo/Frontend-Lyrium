                ...
                {buyers.map((buyer, idx) => (
                    <div key={buyer.id} className="flex items-center gap-4 p-4 rounded-2xl hover:bg-[var(--bg-secondary)] transition-all group">
                        ...
                        <div className="flex-1">
                            <p className="text-sm font-black text-[var(--text-primary)] uppercase group-hover:text-[var(--brand-green)] transition-colors">{buyer.name}</p>
...
export const SalesHeatmap: React.FC<{ data: HeatmapData[] }> = ({ data }) => {
    ...
    const getBgColor = (intensity: number) => {
        ...
        if (intensity < 40) return 'bg-emerald-500/20';
        if (intensity < 70) return 'bg-emerald-500/40';
        if (intensity < 100) return 'bg-emerald-500/60';
        ...
        <div className="bg-[var(--bg-card)] p-8 rounded-[2.5rem] border border-[var(--border-subtle)] shadow-xl font-industrial col-span-full lg:col-span-2">
            ...
            <div className="mt-8 flex items-center justify-end gap-4">
                ...
                <div className="flex items-center gap-1">
                    ...
                    <div className="w-3 h-3 bg-emerald-500/20 rounded shadow-sm"></div>
                    <div className="w-3 h-3 bg-emerald-500/40 rounded shadow-sm"></div>
                    <div className="w-3 h-3 bg-emerald-500/60 rounded shadow-sm"></div>
                    <div className="w-3 h-3 bg-emerald-700 rounded shadow-sm"></div>
                </div>
            </div>
        </div>
    );
