...
export default function BrandsCarousel({ marcas }: BrandsCarouselProps) {
  ...
  return (
    ...
    >
      ...
      <div
        ref={containerRef}
        className="relative overflow-hidden cursor-grab active:cursor-grabbing -mx-4 px-4"
        ...
        >
          {marcas.map((marca) => (
            <Link
              ...
              className="block flex-shrink-0 w-1/2 sm:w-1/3 lg:w-1/4 xl:w-1/5 snap-start p-2"
            >
              <div className="h-38 md:h-46 flex items-center justify-center p-1 bg-white dark:bg-[var(--bg-card)] w-full rounded-2xl overflow-hidden border border-gray-200 dark:border-[var(--border-subtle)] shadow-sm">
